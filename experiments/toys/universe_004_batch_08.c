...
// DeepSeek ~ 2025.07.31
// gcc universe_004_batch_08.c -o universe_004_batch_08 -lSDL2 -lm
// chaosregular(x)2025-07-31_16:38:39 debugging
//  245 |             tmp_cf = fabs(f/last_max_f);
// chaosregular(x)2025-08-04_04:48:02 added energy from Seek suggestion
// dodane masy nowych(n = 1,m = 2) (universe_004_batch_07.c)
// chaosregular(x)2025-08-10_15:22:07
// dynamiczne tłumienie (universe_004_batch_08.c)
// chaosregular(x)2025-08-18_21:53:11 poprzenoszone na koniec main i update physics
#include <SDL2/SDL.h>
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
...
#define MASS_0 33e-2
#define MASS_1 2.0 * MASS_0
#define ATN_COEF_0 0.995 // 0.995
#define WIDTH 1300
#define HEIGHT 850
...
//    double rr = r2 * r2 * r2 * r;     // r^7
    return G_CONSTANT_ATTRACTIVE * m1 * m2 / r2 - G_CONSTANT_REPULSIVE / rr;
}
...
void update_title(SDL_Window* window) {
    if (SDL_GetTicks() - last_title_update > 250) {
        ...
        snprintf(title, sizeof(title), 
            "3D Force-Flip Universe | Scale: %.2f | dt: %.6f | Part: %d | Steps: %d | Time: %.2fs | iN %i | E ~ %f",
            view_scale, time_step, particle_count, step_count, total_simulation_time, setn, tot_energy_est);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_KEYDOWN) {
                if (event.key.keysym.sym == SDLK_n) add_particle(MASS_0);
                if (event.key.keysym.sym == SDLK_m) add_particle(MASS_1);
